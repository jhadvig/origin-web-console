...
angular.module("openshiftConsole")
  .factory("AuthorizationService", function($q, $cacheFactory, Logger, $interval, APIService, DataService){
    ...
    // Transform data from:
    // rules = {resources: ["jobs"], apiGroups: ["batch"], verbs:["create","delete","get","list","update"]}
    // into:
    // normalizedRules = {"batch": {"jobs": ["create","delete","get","list","update"]}}
    ...
    var canI = function(verb, resource, args) {
      if (permissiveMode) {
        return true;
      }
      ...
      r.group = args ? args.group : '';
      var rules = getRulesForProject(args ? args.projectName : currentProject);
      if (!rules) {
        return false;
      }
